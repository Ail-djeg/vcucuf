...
def S1():
    ...
    while True:
        try:
            ...
            j = ''.join(random.choice(k) for _ in range(random.randrange(4, 9)))
            i = "".join(random.choice('1234567890') for _ in range(19))
            headers = {
                "User-Agent": f'com.zhiliaoapp.musically/{j} (Linux; U; Android {random.randrange(7,13)}; en_IQ_#u-nu-letn; ANY-LX1; Build/{j}; Cronet/58.0.{random.randrange(3,9)}.0)'
            }
            ...
            for q in r.get('user_list', []):
                s += 1
                fo = q['user_info'].get('follower_count', 0)
                if fo > 500:
                    u = q['user_info'].get('unique_id', '')
                    color = colors[s % len(colors)]
                    reset = '\033[0m'
                    print(f'{color}{s} : {u} : {fo}{reset}')
                    with open('Wp.txt', 'a', encoding='utf-8') as rk:
                        rk.write(u + '\n')

        except Exception as e:
            # يمكن تفعيل الطباعة لمراقبة الأخطاء
            # print(f'Error: {e}')
            pass
